...
export function LeadSection() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus("Solicitação enviada! Entraremos em contato.");
        setLastSubmit(now);
        (e.target as HTMLFormElement).reset(); // Limpa o form

        // Opcional: Redirecionar para o Zap do Eduardo após 2 seg
        setTimeout(() => {
          const msg = `Olá! Sou ${data.nome} e acabei de enviar uma solicitação pelo site.`;
          window.open(`https://wa.me/55NUMERO_DO_EDUARDO?text=${encodeURIComponent(msg)}`, "_blank");
        }, 2000);
      ...
      <div className="max-w-4xl mx-auto bg-zinc-900/40 border border-white/5 p-8 md:p-16 rounded-[2.5rem] backdrop-blur-xl relative z-10 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="hidden lg:flex flex-col justify-end pb-2">
             <p className="text-[9px] text-zinc-600 uppercase tracking-tighter">Analisamos seu negócio em até 24h.</p>
          </div>
